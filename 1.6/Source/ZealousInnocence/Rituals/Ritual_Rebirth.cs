        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            if (!job.GetTarget(TargetIndex.A).Thing.DestroyedOrNull())
            {
                ...
                yield return Toils_General.Do(delegate
                {
                    ...
                    {
                        //RimWorld.RitualObligationTargetWorker_AnyEmptyGrave.LabelExtraPart
                        ...
                        Helper_Regression.regressOrReincarnateToChild(innerPawn,this.Corpse.def, false, out hediffsRemoved, out ageDifference);                        
                    }
                    else
                    {
                        Messages.Message("MessagePawnResurrectedFailed".Translate(innerPawn), innerPawn, MessageTypeDefOf.PositiveEvent);
                    }
                });
            }
        }
