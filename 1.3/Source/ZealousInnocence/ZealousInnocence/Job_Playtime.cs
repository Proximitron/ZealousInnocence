...
namespace ZealousInnocence
{
    ...
    public class InteractionWorker_RegressedPlayTime : InteractionWorker
    {
        public override float RandomSelectionWeight(Pawn initiator, Pawn recipient)
        {
            ...
            if(!RegressionHelper.isAgeStage(recipient, 10))
            {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            else
            {
                ...
                else if (Rand.ChanceSeeded(0.8f, pawn.HashOffsetTicks()))
                {
                    ...
                    if (CellFinder.TryFindRandomReachableCellNear(pawn.Position, pawn.Map, 12f, TraverseParms.For(TraverseMode.NoPassClosedDoors), (IntVec3 x) => validator2(x), null, out var result2))
                    {
                ...
                else
                {
                    ...
                    if (CellFinder.TryFindRandomReachableCellNear(pawn.Position, pawn.Map, 12f, TraverseParms.For(TraverseMode.NoPassClosedDoors), (IntVec3 x) => validator(x), null, out var result3))
                    {
                        if ((pawn.Position - friend.Position).LengthHorizontalSquared >= 5 || (LovePartnerRelationUtility.LovePartnerRelationExists(pawn, friend) && pawn.Position != friend.Position))
                        {
                            pawn.mindState.nextMoveOrderIsWait = !pawn.mindState.nextMoveOrderIsWait;
